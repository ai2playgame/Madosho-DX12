...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	if (FAILED(result)) {
		if (result == HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND)) {
			...
			std::string errstr;
			// GetBufferSize()にはエラーメッセージが入っている
	...
	// 頂点レイアウトの設定（GPUにバイナリデータを頂点データだと正しく認識させる）
	...
	// SampleMaskの設定（The sample mask for the blend state.)
	...
	// BlendState.RenderTarget[0]に割り当てるブレンドステート
	...
	// レンダーターゲットに関する設定
	gpipeline.NumRenderTargets = 1;// 今は１つのみ。マルチレンダーターゲットではないため
	gpipeline.RTVFormats[0] = DXGI_FORMAT_R8G8B8A8_UNORM;// 0～1に正規化されたRGBA

	// アンチエイリアシングの設定（今回は切っている）
