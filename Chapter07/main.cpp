...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	result = _dev->CreateCommittedResource(
		...
		&_depthClearValue,	// 初期値（普段はnullptrだが深度バッファは初期値が必要）
	...
	struct PMDMaterial {
		...
		// 2バイトのパディングが発生！！しないように#pragma pack(1)
...
#pragma pack()//1バイトパッキング解除
	...
	materialBuffSize = (materialBuffSize + 0xff) & ~0xff;	// 256アライメントに設定。メモリ相当無駄にしている（p262参照）
	...
	gpipeline.DepthStencilState.StencilEnable = false; // ステンシルは今は使わない
	...
	while (true) {
		...
		auto dsvH = dsvHeap->GetCPUDescriptorHandleForHeapStart(); // 深度ステンシルビューのハンドルを取得
