...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	// POSITION: xyz座標
	// TEXCOORD: uv座標
	// HLSLで記述した頂点シェーダはSemanticsNameを参照して頂点データの入力を受け取る
	D3D12_INPUT_ELEMENT_DESC inputLayout[] = {
		{ "POSITION",0,DXGI_FORMAT_R32G32B32_FLOAT,0,D3D12_APPEND_ALIGNED_ELEMENT,D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA,0 },
		{ "TEXCOORD",0,DXGI_FORMAT_R32G32_FLOAT,0,D3D12_APPEND_ALIGNED_ELEMENT,D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA,0 },
	};
	
	// グラフィックパイプライン構築用のdesc構造体を設定
	...
	// ディスクリプタレンジを作成（ディスクリプタヒープ上のディスクリプタをまとめて扱うための構造体）
	...
	// ルートパラメータ（ディスクリプタテーブルの本体）を作る
	D3D12_ROOT_PARAMETER rootparam{};
	rootparam.ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	rootparam.DescriptorTable.pDescriptorRanges = &descTblRange; // ディクリプタレンジのアドレス
	...
	texHeapProp.CPUPageProperty = D3D12_CPU_PAGE_PROPERTY_WRITE_BACK; // CPUからGPUへのデータ転送はライトバック方式
	...
	resDesc.SampleDesc.Quality = 0; // 同情
	...
	// texDescHeap（ディスクリプタヒープ）上に実際にシェーダーリソースビューを作る
	...
	while (true) {
		...
		_cmdList->SetGraphicsRootSignature(rootsignature);	// ルートシグネチャを設定するコマンド
		_cmdList->SetDescriptorHeaps(1, &texDescHeap);	// ディスクリプタヒープを指定する（今回は1つ）
		_cmdList->SetGraphicsRootDescriptorTable(0,	// ルートパラメータ（ディスクリプタテーブルの本体）のインデックス
											     texDescHeap->GetGPUDescriptorHandleForHeapStart());	// ヒープアドレスを指定
